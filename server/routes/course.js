...
router.get('/getRegisteredCourses', upload.none(), async (req, res) => {
    try {
        ...
        const query = 'select _id as course_id, title from courses where tutor_id = 1 union select crs._id as course_id, crs.title as title from coursestudentrelationships as csr join courses as crs on csr.course_id = crs._id where student_id = $1;'
